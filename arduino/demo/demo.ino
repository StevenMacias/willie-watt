...
#define WATER_PUMP  10      // CHANGE THIS TO 8 TO ACTUALLY ASSIGN IT TO THE WATER PUMP
#define HEATER      8       // CHANGE THIS TO 10 TO ACTUALLY ASSIGN IT TO THE HEATER
...
#define WATER_PUMP_TIME_ON 3000
#define HEATER_TIME_ON 180000
...
enum State_enum {IDLE, LOAD_WATER, HEAT_WATER};
...
void loadWaterFunction()
{
  unsigned long now = millis();
  if(start)
  {
    if(now-prev_time > WATER_PUMP_TIME_ON)
    {
      state = HEAT_WATER;
      turnOFFWaterPump();
...
void heaterFunction()
{
  ...
  if(start)
  {
    if(now-prev_time > HEATER_TIME_ON)
    {
      turnOFFHeater();      
      digitalWrite(13, LOW);
      state = IDLE;
...
{
  unsigned long now = millis();
  if(now-prev_blink_time > 500)
    {
      prev_blink_time = now;
      blink_state = !digitalRead(13);
    }else
    {
